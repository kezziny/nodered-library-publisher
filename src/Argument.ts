declare type ArgumentMapSettings<T extends { [key: string]: any }> = {
  [P in keyof T]: ArgumentSettings;
};
...
export class Argument {
  ...
  constructor(schema: ArgumentSettings, field: string = "Data") {
    this.field = field;
  ...
  eval(data, context = {}) {
    ...
    if (this.schema.array) {
      ...
      if (this.schema.arraySizeMin && data.length < this.schema.arraySizeMin) {
        throw new Error(
          `${this.field} array must have at least ${this.schema.arraySizeMin} element`,
        );
      }

      if (this.schema.arraySizeMax && data.length > this.schema.arraySizeMax) {
        throw new Error(
          `${this.field} array must have at most ${this.schema.arraySizeMax} element`,
        );
      }
      ...
      data.forEach((value, index) => {
        try {
          ...
          throw new Error(
            `${this.field} array element #${index}: ${e.message}`,
          );
        }
      });
    } else {
  ...
  validateElement(data) {
    ...
    if (!(typeof this.schema.type === 'string')) {
      if (!(data instanceof this.schema.type)) {
        throw new Error(
          `${this.field} must be instance of ` +
      ...
      switch (this.schema.type) {
        ...
        case 'number': {
          if (typeof data !== 'number') {
            throw new Error(`${this.field} must be type of number`); // TODO type: Number?
          }
        ...
        default: {
          ...
          if (!options.find((o) => o === data)) {
            throw new Error(
              `${this.field} (${data}) must be type of string and one of the following options: ${options}`,
            );
          }
        }
      }
...
export class ArgumentMap {
  public schema: { [key: string]: Argument };
  ...
  constructor(schema: {
    ...
    for (const key in schema) {
      if (schema[key] instanceof Argument || schema[key] instanceof ArgumentMap)
        continue;
      schema[key] = new Argument(schema[key] as ArgumentSettings, key);
    }
  ...
  eval(data) {
    ...
    Object.getOwnPropertyNames(this.schema).forEach((key) => {
      if (typeof this.schema[key].schema.default === 'function') return;
      try {
